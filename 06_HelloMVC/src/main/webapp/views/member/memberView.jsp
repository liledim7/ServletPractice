...
<section id=enroll-container>
	...
	<form id="memberFrm" method="post">
		<table>
			<tr>
				<th>아이디</th>
				<td><input type="text" name="userId" id="userId_" value="<%=infoMember.getUserId()%>" readonly ></td>
			</tr>
			<!-- <tr>
				<th>패스워드</th>
			...
			<tr>
				<th>이름</th>
				<td><input type="text" name="userName" id="userName" required value="<%=infoMember.getUserName() %>"><br>
				</td>
			</tr>
			<tr>
				<th>나이</th>
				<td><input type="number" name="age" id="age" value="<%=infoMember.getAge()%>"><br>
				</td>
			</tr>
			<tr>
				<th>이메일</th>
			...
			<tr>
				...
				<td><input type="tel" placeholder="(-없이)01012345678"
					name="phone" id="phone" maxlength="11" value="<%=infoMember.getPhone() %>"><br></td>
			</tr>
			<tr>
				<th>주소</th>
				<td><input type="text" placeholder="" name="address"
					id="address" value="<%=infoMember.getAddress()%>"><br></td>
			</tr>
			<tr>
				<th>성별</th>
				<td><input type="radio" name="gender" value="M"<%=infoMember.getGender()=='M'?"checked":"" %>
					id="gender0" > <label for="gender0">남</label>
					 <input type="radio" name="gender" id="gender1" value="F"<%=infoMember.getGender()=='F'?"checked":"" %>> <label
					for="gender1">여</label>
			...
			<tr>
				<th>취미</th>
				<td><input type="checkbox" name="hobby" id="hobby0" value="운동" <%=checkData[0] %>><label for="hobby0">운동</label>
					...
					<input type="checkbox" name="hobby" id="hobby4" value="여행" <%=checkData[4] %>><label for="hobby4">여행</label><br></td>
			</tr>
		</table>
...
<script>
	...
	const fn_updatePassword=()=>{
		...
		<%-- $("#memberFrm").attr("action","<%=request.getContextPath()%>/member/updateEndPassword.do").submit(); --%>
	}
</script>

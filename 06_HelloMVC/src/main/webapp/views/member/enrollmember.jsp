...
<script>
	function fn_validate2(){
		const user=$("#userId_").val();
		if(userId.length<4){
			alsert("아이디는 4글자 이상으로 작성하세요");
			$("#userId_").focus;
			return false;
		}
	};
	...
	$("#userId_").keyup(e=>{
		if(e.target.value.length>=4){
			$.ajax({
			url:"<%=request.getContextPath()%>/ajaxDuplicated.do",
		...
		<table>
			<tr>
				...
				<td><input type="text" placeholder="4글자이상" name="userId"
					id="userId_"><input type="button" onclick="fn_duplicateId();" value="중복확인"></td>
					<!-- 새 창 -> /member/idduplate.do 
					    새창에서 id중복확인을 할 수 있는 페이지
					    1)클라이언트가 id를 전송 2) 중복확인 3) 결과를 페이지에 전송
					    중복되면 다시 중복확인 of 아이디와 확인버튼-->
			</tr>
			<tr>
				<th>패스워드</th>
			...
			<tr>
				...
				<td><input type="radio" name="gender" id="gender0" value="M"> <label
					for="gender0">남</label> <input type="radio" name="gender" value="F"
					id="gender1"> <label for="gender1">여</label></td>
			</tr>
			<tr>
				<th>취미</th>
